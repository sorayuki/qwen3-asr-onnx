...
因此显存锯齿不是 Python 层显式卸载、重载模型。适配前更可能的来源是：

- TensorRT 针对新动态 shape 构建或更新 engine/profile；
- ORT 在 TensorRT、CUDA fallback 之间进行图分区和内存搬运；
- 原来的普通 `session.run()` 把 KV cache 输出复制到 CPU NumPy，下一轮又上传 GPU；
...
- GPU 常驻 KV 的标准可比 benchmark 中 decode 为 9.93 ms/token。
...
|---|---|---:|---:|---:|---:|---:|
| CUDA | single | no | 15.459x | 17.50 ms/token | 6 | 47 |
| TensorRT decoder + CUDA support graphs | single | no | 22.215x | 9.93 ms/token | 8 | 47 |

这组数据可以直接比较，因为音频、模型、decoder graph、生成上限、EOS 行为、warmup、I/O Binding 和计时规则均相同。TensorRT single 的吞吐比 CUDA single 高约 44%，逐 token decoder 延迟低约 43%。

## 12. GPU 常驻 KV cache 与后续优化

### 12.1 为什么普通 session.run 很慢
...
这不会复制模型权重，但每个 token 都要传输 28 层、共 56 个不断增长的 KV 张量。以本模型的 `[1,8,seq,128]` FP16 cache 计算，每增加一个序列位置，全部层的 key/value 合计增长 112 KiB。传输量会随序列长度线性增长，并在整个生成循环中累计。

这也是旧 benchmark 中 TensorRT 比 CUDA 慢的主要原因。TensorRT 除了传输数据，还要为 56 个动态 cache 输入重新绑定 shape；辅助图使用 CUDA 并不是主要矛盾。

### 12.2 I/O Binding 实现

GPU decoder 现在通过 `run_decoder()` 使用 ORT I/O Binding：

```text
NumPy inputs_embeds / position_ids / attention_mask
    -> bind_cpu_input

previous CUDA present KV
    -> bind_ortvalue_input as next past KV
...
`binding.get_outputs()` 返回 CUDA `OrtValue`。程序只对 CPU logits 调用 `.numpy()` 做 greedy argmax，不调用 `copy_outputs_to_cpu()`，因此 56 个 present cache 保持在 GPU，并在下一轮直接绑定。旧 cache 在新一轮完成并替换引用后释放。

该路径对 CUDA、TensorRT 和 DirectML 自动启用。DirectML provider 的 `device_id` 选择 DXGI adapter，但 ORT 为所选 session 注册的 I/O Binding allocator 始终名为 `dml:0`；不能把 adapter id 原样当成 OrtDevice id。CPU、OpenVINO 继续使用普通 NumPy 路径。

### 12.3 Int64 binding 警告
...
10. KV cache 使用 I/O Binding 后，再优化 int32 输入和更细的 profile/bucket 策略。
